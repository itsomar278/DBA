...
ALTER TABLE Staff
ADD branch_id INT, FOREIGN KEY (branch_id) REFERENCES Branch(branch_id);
...
CREATE TABLE BookRent (
    ...
    is_returned VARCHAR(5) DEFAULT 'false' CHECK (is_returned IN ('true', 'false')),
    FOREIGN KEY (customer_id) REFERENCES Customer(id),
    FOREIGN KEY (ISBN, branch_id) REFERENCES Stock(ISBN, branch_id)
);

ALTER TABLE BookRent
ADD is_returned VARCHAR(5) DEFAULT 'false' CHECK (is_returned IN ('true', 'false'));
